...
커스텀 토글은 켜짐/꺼짐 상태를 오래 유지해야 하거나, 상태마다 다른 모습이 필요하거나, 스위치가 움직이는 듯한 효과가 필요할 때 사용합니다.
...
`common.toggle`을 범용 이벤트 발생기처럼 억지로 쓰지 마세요. `toggle_on_button`, `toggle_off_button`은 토글이 듣는 입력 이벤트에 가깝고, 다른 컨트롤로 신호를 내보내는 안정적인 API가 아닙니다.
...
바로 복사해서 구조를 볼 수 있는 토글 스니펫은 [예제 코드](../examples/index.md#커스텀-토글)에 넣어 두었습니다.

## 애니메이션 토글 착시

on/off 애니메이션은 문제를 나누는 편이 안정적입니다.

- 실제 토글이 `#toggle_state`를 소유
- 바인딩이 상태를 `#collection_length`로 변환
- factory가 들어오는 자식 또는 나가는 자식을 생성
- 새 자식이 생성되면서 애니메이션 재생
...
마우스를 올렸을 때와 포커스됐을 때의 모습은 아래 상태 컨트롤이 담당합니다.
...
포커스 테두리 색을 바꾸고 싶다면 실제 상태 컨트롤을 교체하세요. 하나의 마법 같은 focus-color 속성을 찾는 방식은 안정적이지 않습니다.
...
3. `play_event`를 쓸 때 애니메이션을 담는 바깥 패널을 안정적으로 유지합니다.
