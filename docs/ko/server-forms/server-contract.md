...
커스텀 서버폼은 서버 코드와 리소스팩이 같은 규칙을 공유할 때만 안정적으로 동작합니다. 여기서 말하는 규칙은 어려운 설계 문서가 아니라, "어떤 제목 토큰이면 어떤 UI를 띄우고, 몇 번째 버튼이 어떤 기능인지"를 정해 둔 표입니다.

<img class="jsonui-diagram" src="/assets/diagrams/server-contract.svg" alt="서버폼 계약 다이어그램">

이 규칙은 아래 네 가지 질문에 답해야 합니다.
...
BP Script의 beta API는 문법이 바뀔 수 있습니다. UI 연동 규칙은 API 호출 문법과 분리해서 관리하세요. 폼 빌더 호출이 바뀌어도 제목 토큰, 버튼 순서, 필드 순서는 그대로 유지되어야 합니다.
